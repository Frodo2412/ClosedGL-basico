...
bool plane::test_intersection(ray& rayo, vector3& point, vector3& normal)
{
    ...
    if (t > 0.0)
    {
        ...
        double u = (pt.get_z() * V_.get_x() - pt.get_x() * V_.get_z()) / (U_.get_z() * V_.get_x() - U_.get_x() * V_.
            get_z());
        double v = (pt.get_x() * U_.get_z() - pt.get_z() * U_.get_x()) / (U_.get_z() * V_.get_x() - U_.get_x() * V_.
            get_z());
        ...
        {
            u = (pt.get_y() * V_.get_z() - pt.get_z() * V_.get_y()) / (U_.get_y() * V_.get_z() - U_.get_z() * V_.
                get_y());
            v = (pt.get_z() * U_.get_y() - pt.get_y() * U_.get_z()) / (U_.get_y() * V_.get_z() - U_.get_z() * V_.
                get_y());
        }
        if (normal_ == vector3(0, 1, 0) || normal_ == vector3(0, -1, 0))
        {
            u = (pt.get_x() * V_.get_z() - pt.get_z() * V_.get_x()) / (U_.get_x() * V_.get_z() - U_.get_z() * V_.
                get_x());
            v = (pt.get_z() * U_.get_x() - pt.get_x() * U_.get_z()) / (U_.get_x() * V_.get_z() - U_.get_z() * V_.
                get_x());
        }
        if (normal_ == vector3(0, 0, 1) || normal_ == vector3(0, 0, -1))
        {
            u = (pt.get_x() * V_.get_y() - pt.get_y() * V_.get_x()) / (U_.get_x() * V_.get_y() - U_.get_y() * V_.
                get_x());
            v = (pt.get_y() * U_.get_x() - pt.get_x() * U_.get_y()) / (U_.get_x() * V_.get_y() - U_.get_y() * V_.
                get_x());
        }
